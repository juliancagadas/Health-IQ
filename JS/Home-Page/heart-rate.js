...
function getHeartRateFuzzy(heartRate) {
    ...
    if (heartRate <= 30) {
        ...
        heartNormal = (heartRate - 30) / 40;
    }
    
    else if (heartRate <= 110) {
        heartNormal = (110 - heartRate) / 40;
    }
...
heartSlider.addEventListener("input", () => {
    ...
    heartvalueBox.textContent = heartRate + " BPM";
    // Get fuzzy values
    const fuzzy = getHeartRateFuzzy(heartRate);


    // =================================================
    // GRAPH VALUES
    // =================================================
    document.querySelector("#graph-heart-low").textContent = (fuzzy.low * 100).toFixed(0) + "%";
    document.querySelector("#graph-heart-normal").textContent = (fuzzy.normal * 100).toFixed(0) + "%";
    document.querySelector("#graph-heart-high").textContent =(fuzzy.high * 100).toFixed(0) + "%";
...
window.getHeartRateFuzzy = getHeartRateFuzzy;
...
heartSlider.dispatchEvent(new Event("input"));